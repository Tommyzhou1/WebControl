...
function InitializeBlock() {
    for (let i = 1; i < numBlockCol; i++) {
        for (let j = 0; j < numBlockRow; j++) {
            blockArr[blockIndex] = new Block(padding / 2 + i * (colWidth) - colWidth / 2, 0.7 * colWidth + j * (
                colWidth - 2 * padding), blockProp.w - padding, blockProp.h, true);
            blockIndex++;
        }
    }
}
    ...
    blockArr.forEach((b) => {
        if (isBallhitsBlock(ball, b) && b.isShow) {
            ...
            if (numObBlocksHit == (numBlockCol - 1) * numBlockRow) {
                ...
                document.getElementById('gameover').innerHTML = 'Game Level ' + gameLevel +
...
Ball.prototype.updateBall = function (playerObj) {
    ...
    if (this.x + this.radius > playerObj.x && this.x - this.radius < (playerObj.x + playerObj.width) && this
        .y + this.radius > (HEIGHT - (1.5 * padding))) {
        this.yDirSpeed = -this.yDirSpeed;
        this.y += this.yDirSpeed;
    ...
    if ((this.x + this.radius < playerObj.x || (this.x + this.radius) > (playerObj.x + playerObj.width)) &&
...
function isBallhitsBlock(ball, block) {
    if (ball.x + ball.radius > block.x && (ball.x - ball.radius < (block.x + block.width)) && ball.y + ball
        .radius > block.y && ball.y - ball.radius < (block.y + block.height)) {
        return true;
    } else {
        false;
    }
